...
require_once __DIR__ . '/gamephp/workerman/Autoloader.php';
error_reporting(E_ALL & ~E_NOTICE);
ini_set('date.timezone','Asia/Shanghai');
//包含数据库操作类文件
include './gamephp/mysql.class.php';
include './gamephp/config.php';
...
$worker->onWorkerStart = function($worker)
{
    ...
    foreach ($serverlsit as $key => $value) {
        ...
        if($value['type']!='0'){
            $source = "./gamephp/game{$value['type']}.php";
            $target = "./gamephp/runtime{$value['id']}-game{$value['type']}.php";
            if(file_exists($target)) unlink($target);
            copy($source, $target);
            $command .= "php {$target} start -d {$id} {$value['dk']} {$title}\n";
        }
        else{
            $command .= "php ./gamephp/test.php start -d {$id} {$value['dk']} {$title}\n";
        }
    }
...
function reqact($data2,$connection){
    ...
    if($connection->sfxt==1){
        $tpl=file_get_contents("./gamephp/sys/sact/".$data2['act'].".php");
    }
    elseif($connection->task){
        $tpl=file_get_contents("./gamephp/sys/task/".$data2['act'].".php");
    }
    else{
        $tpl=file_get_contents("./gamephp/sys/act/".$data2['act'].".php");
    }
