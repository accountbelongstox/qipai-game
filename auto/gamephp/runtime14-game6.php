...
function reqact($data2,$connection){
    ...
    global $extract;
    global $jimi;
    $tpl=file_get_contents("./gamephp/game6/".preg_replace('/[^a-zA-Z0-9]/is','',$data2['act']).".php");
    if($jimi){
...
function typexx($card,$bs){
    ...
    elseif( ($card['0']['card']+$card['1']['card']==100 && $card['0']['card']==20 && $bs!=1) || ($card['0']['card']+$card['1']['card']==100 && $card['1']['card']==20 && $bs!=1)){
        $result['type']='10';
        $result['typename']='二八杠';
        $result['bs']=$bs;
